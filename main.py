...
def gauss_elimination(A, b):
    ...
    for iter in range(n):
        max_row = iter + max(range(n - iter), key=lambda k: abs(A[iter + k][iter]))


        A[iter], A[max_row] = A[max_row], A[iter]
        b[iter], b[max_row] = b[max_row], b[iter]


        for j in range(iter + 1, n):
            factor = A[j][iter] / A[iter][iter]
            for k in range(iter, n):
                A[j][k] -= factor * A[iter][k]
            b[j] -= factor * b[iter]

    x = [0 for _ in range(n)]
    for iter in range(n - 1, -1, -1):
        x[iter] = (b[iter] - sum(A[iter][j] * x[j] for j in range(iter + 1, n))) / A[iter][iter]
